...
interface GiveawayContextType {
  giveaways: Giveaway[];
  analytics: Analytics;
  createGiveaway: (giveaway: Omit<Giveaway, 'id' | 'createdAt' | 'updatedAt' | 'entries'>) => Promise<void>;
...
export function GiveawayProvider({ children }: { children: React.ReactNode }) {
  ...
  const createGiveaway = async (giveawayData: Omit<Giveaway, 'id' | 'createdAt' | 'updatedAt' | 'entries'>) => {
